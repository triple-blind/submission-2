...
void NetworkThread::run() {
	cout << "The network thread is running!" << endl;
}
...
OUString getPathToExtensionRoot(const char* name) {
	...
	Reference<XComponentContext> localContext = defaultBootstrap_InitialComponentContext();
	Reference<XPackageInformationProvider> xPackageInformationProvider = PackageInformationProvider::get(localContext);
...
OUString getPathToExtensionRoot() {
	return getPathToExtensionRoot("org.openoffice.legacy.complextoolbarcontrols.oxt");
}

OUString getPathToExtensionRoot(const char* name, const Reference<XComponentContext> context) {
	OUString ouName = OUString::createFromAscii(name);
	Reference<XPackageInformationProvider> xPackageInformationProvider = PackageInformationProvider::get(context);
	OUString path = xPackageInformationProvider->getPackageLocation(ouName);
	return path;
}

OUString getPathToExtensionRoot(const Reference<XComponentContext> context) {
	return getPathToExtensionRoot("org.openoffice.legacy.complextoolbarcontrols.oxt", context);
}
...
void SAL_CALL BaseDispatch::dispatch( const URL& aURL, const Sequence < PropertyValue >& lArgs ) throw (RuntimeException)
{
	...
	if ( !aURL.Protocol.compareToAscii( RTL_CONSTASCII_STRINGPARAM( "vnd.demo.complextoolbarcontrols.demoaddon:" ) ) )
	{
		if ( !aURL.Path.compareToAscii( RTL_CONSTASCII_STRINGPARAM( "ImageButtonCmd" ) ) )
		{
            ...
            {
				// kwa
				getPathToExtensionRoot(mxContext);
