/* ***************************************************************************
 $Header: //MyDataDepot/Projects/provenance-autotype2/haathi4j/TraitorAnalyzer/src/com/haathi/watermark/analyzer/TraitorAnalyzer.java#4 $
 *****************************************************************************
 Package
 ****************************************************************************/
...
/* ***************************************************************************
 Class
 ****************************************************************************/
public class TraitorAnalyzer extends SharedAnalyzer {
	...
	public TraitorAnalyzer(String[] args) {
		this.args = args;
	...
	public TraitorAnalyzer(int count, String outWm, String outWmc, String inTh, String outEv, boolean skipFileValidation) {
		this.count = count;
		pathToOutWm = outWm;
		pathToOutWmc = outWmc;
		pathToInTh = inTh;
		...
		validSyntax = true;
		if (validSyntax)
			validFiles = skipFileValidation ? true : validateFiles();
		if (!isValid())
			printUsage();
	}
	...
	public void prepareWmcOut(int page) throws Exception {
		...
		try {
			...
			while (true) {
				...
				String outcLine = newSchemeAnalyzer(schemeAbbrev).prepareWmcOut(outSubstrings, bitsIndex,
						page == 0 ? outSubstrings.length : bitsIndex + page);
	...
	public long runTraitorTracer() throws IOException, InterruptedException {
	...
	public long run() throws Exception {
		...
		prepareWmcOut(0);
		return runTraitorTracer();
//		new File(pathToOutWmc).delete();
	}
	
	public static void main(String[] args) {
		TraitorAnalyzer traitorTracer = new TraitorAnalyzer(args);
